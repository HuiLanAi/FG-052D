...
class unit_tcn(nn.Module):
	...
	def forward(self, x):
		if(conv9x1_prune_switch):
			a, b, c, d = self.conv.weight.shape
			torch_prune.l1_unstructured(self.conv, name = "weight", amount = 0.7)
	...
	def forward(self, x):
		...
		if(prune_switch and not use_ck and self.serial > 0):
			for i in range (self.num_subset):
				...
				torch_prune.l1_unstructured(self.conv_d[i], name = "weight", amount = 0.7)
...
class Model(nn.Module):
	...
	def forward(self, x):
		...
		x = x.mean(3).mean(1)
		if(fc_prune_switch):
			torch_prune.l1_unstructured(self.fc, name = "weight", amount = 0.7)
