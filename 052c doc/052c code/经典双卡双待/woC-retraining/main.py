...
class Processor():
    ...
    def load_model(self):
        ...
        self.model = nn.DataParallel(self.model, device_ids = self.device_ids, output_device = 1)
        self.model.to(self.device)
        # self.model = self.model.cuda(1)
        # self.model.to([1])

        self.loss = nn.CrossEntropyLoss().cuda(1)
        ...
        if self.arg.weights:
            ...
            weights = OrderedDict(
                [["module." + k,
                  v.to(1)] for k, v in weights.items()])
            ...
            for k, v in weights.items():
                if("weight" in k or "bias" in k):
                    weights[k].to(1).detach().requires_grad_(True)
