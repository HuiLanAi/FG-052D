...
def prune(weight):
	...
	if(out_flag == 0):
		...
		zero_mtx = torch.zeros(o_c, in_c, w, h).cuda(1)
...
def max_seg_prune(weight):
	...
	if(in_c == 3):
		...
		with torch.no_grad():
			...
			tmp = torch.abs(pruned_weight) < max_val.cuda(1)
			pruned_weight[tmp] = 0
	...
	def forward(self, x):
		...
		for i in range(self.num_subset):
			...
			if use_ck:
				...
				# debug = input()
				try:
					A1 = torch.zeros(2, 25, 25).cuda(1)
					A1 = A1 + A[i]
					z = self.conv_d[i](torch.matmul(A2, A1).view(N, C, T, V))
				except RuntimeError:
					try:
						A1 = torch.zeros(48, 25, 25).cuda(1)
						A1 = A1 + A[i]
						z = self.conv_d[i](torch.matmul(A2, A1).view(N, C, T, V))
					except RuntimeError:
						try:
							A1 = torch.zeros(80, 25, 25).cuda(1)
							A1 = A1 + A[i]
							z = self.conv_d[i](torch.matmul(A2, A1).view(N, C, T, V))
						except RuntimeError:
							A1 = torch.zeros(40, 25, 25).cuda(1)
							A1 = A1 + A[i]
							z = self.conv_d[i](torch.matmul(A2, A1).view(N, C, T, V))
