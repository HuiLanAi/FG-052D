...
class Processor():
    ...
    def load_model(self):
        ...
        # print(Model)
        # self.model = Model(**self.arg.model_args).cuda(1)
        ...
        if self.arg.weights:
            ...
            weights = OrderedDict(
                # [[k.split('module.')[-1],
                #   v.cuda(1)] for k, v in weights.items()])
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            self.global_step += 1
            # get data
            # data = Variable(data.float().cuda(1), requires_grad=False)
            # label = Variable(label.long().cuda(1), requires_grad=False)
