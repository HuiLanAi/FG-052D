...
def max_seg_prune(weight):
	...
	if(in_c == 3):
		pruned_weight = weight
		for o_i in range (o_c):
			index = torch.abs(pruned_weight[o_i, :, 0, 0]) < torch.max(torch.abs(pruned_weight[o_i, :, 0, 0]))
			pruned_weight[o_i, index, 0, 0] = 0

	else:
		pruned_weight = weight.permute(0, 2, 3, 1)
		pruned_weight = pruned_weight.reshape(o_c * w, h, in_c)
		pruned_weight = pruned_weight.contiguous().view(o_c * w * h * in_c // 2, 2)

		index_0dim = [a for a in range (0, o_c * w * h * in_c // 2)]
		index_0dim = np.array(index_0dim)
		index_0dim = torch.from_numpy(index_0dim)
		
		max_val = torch.rand((o_c * w * h * in_c // 2, 2))
		
		val, index = torch.max(torch.abs(pruned_weight), 1)
		val = val.reshape(o_c * w * h * in_c // 2, 1)
		max_val[:, 0] = val[:, 0]
		max_val[:, 1] = val[:, 0]
		# max_val[:, 2] = val[:, 0]
		# max_val[:, 3] = val[:, 0]
		...
		pruned_weight = pruned_weight.contiguous().view(o_c, w, h, in_c)
		pruned_weight = pruned_weight.permute(0, 3, 1, 2)
	return pruned_weight
